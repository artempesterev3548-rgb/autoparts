...
export default function ServiceOrderForm({ id, name, phone, linked }: { id: string; name?: string; phone?: string; linked?: string }) {
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      setForm(f => ({
        ...
        client_name: name ?? '',
        client_phone: phone ?? '',
        linked_order_id: linked ? Number(linked) : null,
      ...
      fetch(`/api/admin/service-orders/${id}`)
        .then(r => r.json())
        .then(d => {
          setForm({
            ...EMPTY, ...d,
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/admin/service-orders' : `/api/admin/service-orders/${id}`

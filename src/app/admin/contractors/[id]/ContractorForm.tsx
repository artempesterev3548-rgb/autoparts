...
export default function ContractorForm({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (isNew) return
    Promise.all([
      fetch(`/api/admin/contractors/${id}`).then(r => r.json()),
      fetch(`/api/admin/vehicles?contractor_id=${id}`).then(r => r.json()),
      fetch(`/api/admin/service-orders?contractor_id=${id}`).then(r => r.json()),
    ]).then(([c, v, o]) => {
  ...
  const save = async () => {
    ...
    const url = isNew ? '/api/admin/contractors' : `/api/admin/contractors/${id}`
  ...
  const del = async () => {
    ...
    await fetch(`/api/admin/contractors/${id}`, { method: 'DELETE' })
  ...
  const saveVehicle = async () => {
    setVSaving(true)
    const body = vForm.id
      ? { ...vForm, contractor_id: Number(id) }
      : { ...vForm, id: undefined, contractor_id: Number(id) }
  ...
  const delVehicle = async (vid: number) => {
    if (!confirm('Удалить автомобиль?')) return
    await fetch(`/api/admin/vehicles?id=${vid}`, { method: 'DELETE' })
    ...
    <div style={{ background: '#F0F2F5', minHeight: '100vh', padding: '24px' }}>
      <div style={{ maxWidth: 980, margin: '0 auto' }}>
        ...
        <div style={sectionStyle}>
          <div style={sectionTitleStyle}>Контрагент</div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Условия оплаты, особенности..."
            />
          </div>
        </div>
        ...
        {!isNew && (
          <div style={sectionStyle}>
            <div style={sectionTitleStyle}>Автомобили ({vehicles.length})</div>
            {vehicles.length > 0 && (
        ...
        {!isNew && orders.length > 0 && (
          <div style={sectionStyle}>
            <div style={sectionTitleStyle}>История заказов ({orders.length})</div>
            {orders.map((o, i) => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              {[
                ...
                <div key={f.k}>
                  ...
                  <input
                    style={inp(f.k === 'plate' ? { fontFamily: 'monospace', textTransform: 'uppercase' } : {})}
                    value={(vForm as any)[f.k]}
                    placeholder={f.p}
